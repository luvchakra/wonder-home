...
import { createSchoolItemAction, extractSchoolItemFromPhotoAction, type ExtractSchoolItemState } from "../(auth)/school-actions";
    ...
    <>
      ...
      <Sheet open={open} onOpenChange={setOpen} title="Add a piece of school work" description="WonderHome tracks it against its due date from here — no portal needed.">
        <div className="space-y-4">
          ...
          <form key={prefill ? JSON.stringify(prefill) : "blank"} action={formAction} className="space-y-3">
            ...
            <div className="space-y-1.5">
              ...
              <select
                id="kind"
                name="kind"
                defaultValue={prefill?.kind ?? "homework"}
                className="block min-h-11 w-full rounded-[var(--wh-radius-sm)] border border-[var(--wh-border)] bg-[var(--wh-surface)] px-3 text-base"
              >
                {KINDS.map((k) => (
            ...
            <div className="grid grid-cols-2 gap-3">
              <Field label="Due (optional)" name="dueAt" type="date" defaultValue={prefill?.dueDate ?? ""} />
