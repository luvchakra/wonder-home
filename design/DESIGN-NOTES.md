...
| `CalendarItem`, `PersonCard`, `ResponsibilityCard`, `HandledList` | Domain rows. `PersonCard` carries its person's own tint as the whole card's background (Family screen), not just a border around a white one |
