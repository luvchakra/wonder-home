...
| `Pill` / `PillLink` / `Badge` | The small rounded action or state label. `Badge` grew five domain-coloured tones (`money`/`meals`/`school`/`people`/`home`, reusing the same tokens `IconTile` already carries) alongside its urgency ones (`attention`/`risk`/`handled`), for a row that wants to say *which domain* rather than *how urgent* — Home's Today's-focus rows use a domain tone once an item's risk is too low to earn "Needs attention" |
...
| `CozyCornerIllustration` | A second original illustration (rule 8) — a cat that tiptoes in from the right on arrival, settles, then blinks now and then and flicks its tail (`wh-cat-sneak`/`wh-cat-blink`/`wh-cat-tail` in `ui-theme.css`, transform only, all three removed under `prefers-reduced-motion`, leaving the cat sitting still with its eyes open), a mug, a stack of books and a plant by a sunlit window — for Home's greeting header, with the brand's handwritten line laid over it and a date/household-name chip on top (never an invented temperature: WonderHome has no weather provider, so this shows the one thing it genuinely knows) |
| `NotificationCard`, `CertificationItem`, `ActionPreview` | Threaded notification, a belief with provenance, an approval card. `CertificationItem` (HomeBrain Review) also takes optional `learnedAt` and `confidence`: the fact, its source, when it was learned, a confidence in words (never a bare score, rule 9) and whether anyone confirmed it, per HomeBrain 2.0 §12 |
| `HomeSendFindings`, `TranscriptCheck` (app, `home-send-intake.tsx`) | HomeSend's review step (Wave 3 §13, §17): "What I found" — the summary, the source and when in words ("A PDF you sent · 23 Sep"), confidence as High/Medium/Low (never a bare score, rule 9), and a plain line when the content's instructions to WonderHome were ignored; and the voice-note check that shows what was heard and waits for a person to confirm or retype it. The inbox's "Failed safely" rows use labelled icon buttons (fill in by hand, dismiss) so the name keeps the room (rule 11) |
| HomeSend reconciliation and subject question (app, `home-send-intake.tsx`, Wave 3 §9–§13) | When what was sent is already on record, the review opens with one plain sentence ("I found Asmi's existing Science Exhibition for 28 Sep. This message says it moved to 29 Sep. Update the existing event?") and one path per job (rule 14): *Update existing* / *Cancel existing* in Primary Blue, *Keep existing*, *Add as new* — never two buttons that both mean "add". Who it is for is a picker (rule 20) that starts on "Choose who this is for" and is required when the household has more than one child it could be for, under the one question that says so — never a pre-selected guess. Several needs from one notice ("I found 3 things: …") are checkboxes, each written, named in the inbox and undone on its own (rule 12); a need is never ticked for the person |
| HomeSend confirmation line and "added on its own" (app, `home-send-intake.tsx`/`home-send-inbox.tsx`/`home-send-sheet.tsx`, Wave 3 §12) | The review says *why* it is asking, in words, only when that is news: a bill or a health document carries one quiet line saying it always waits for a person; a low-confidence reading opens with its one question ("Is this something to buy: “Rice”?"). Something the household's own autonomy setting let WonderHome apply arrives as done — an info alert naming what was added and why, with its Undo right there (rule 12) — and stays in "Recently handled" as "· Added on its own", so an automatic write is never mistaken for a person's. "Keep existing" reads back as "Kept existing", not "Dismissed" |
...
| `AiOrb`, `ChatMessage`, `SuggestionChips` | The conversation. `ChatMessage` no longer renders `AiOrb` beside an assistant bubble — alignment (`flex-row`/`items-start`), the neutral surface background and the flat top-left corner already distinguish an assistant turn from a member's without a second visual cue. `AiOrb` itself is unchanged and still used for the quiet-state hero (`assistant.tsx`, 88px) and the landing mockups. Since 2026-09-24 a `sentAt` prop puts the time in the bubble's bottom corner, the way a messaging app does: beside a short message's last words, or on its own line in the corner when the text needs the width. `ChatDayDivider` is the centred date pill between days ("Today", "Yesterday", "September 7, 2026"). Both are formatted by `conversation/message-time.ts` in the household's time zone, never the browser's or the server's, so the server render and the browser's agree and "Today" means the household's today |
| `MobileHeader` `search` / `AppShell` `headerSearch`, `fill` | A screen's own search in the header, in place of its title: the mark beside it on a phone, and in place of the global search on desktop (the page title stays as an sr-only `h1`). First used by HomeTalk's message search (`apps/web/app/ai/conversation-search.tsx`), which only reads: asking something new is still the composer's job (rule 13). `fill` is for a screen that fills the viewport down to the tab bar and scrolls inside itself; it stops `--wh-tabbar-raised-overhang` (the raised button's real ~15px rise) above the bar instead of the full `--wh-tabbar-raised-clearance`, so no empty band sits under its last line (rule 15). Scrolling pages keep the full clearance |
| `TalkComposer` | The one way into the assistant, with four states: type, speak-to-text, send, and a live voice conversation. Replaced `ChatComposer` + `VoiceInputButton` + a toggle, which between them could not say which of two voice intentions a tap meant. An optional `onAttach` prop adds a paperclip button beside the mic (Phase C, HomeSend) — a second input *modality* of the same one door, not a second door: rule 13 still says a screen never gets its own "Ask AI" shortcut, but a photo or a pasted forward is not a question, it is content, so it earns a control here rather than being typed out by hand | A `liveEngine` prop (voice phase 3) picks what runs behind the live control — WonderHome's own listen/speak loop, or Gemini Live via `useGeminiLive` — without a second control: the household chooses once in Voice settings, and the composer shows the same live field, pause and end button either way (rule 14). Since the two-row redesign (2026-09-24) the words take the full width on top and the controls sit in one row beneath: `+` (HomeSend, was the paperclip), the live-engine picker, then the mic and Send. The picker is a Radix `DropdownMenu` radio group offering only WonderHome and Gemini Live (never other model brands — which model answers a typed turn is the household's AI-key setting, not a composer choice); it starts from the household's Voice setting, a member's switch is remembered on that device only, and Gemini Live is disabled with the server's own reason whenever the gate says it cannot run. On a phone the round controls step down to 40px so the row fits 360px without clipping Send (rule 15).
...
| `ComboboxField` | A choice among values a household has already typed — a grocery name, a category, a unit — that must never refuse a genuinely new one. A real `<select>` of the existing values (same OS-picker reasoning as `Select`) ending in "Add new…", which swaps in a plain text field for the value nobody has used yet. Replaces `Field` + `<datalist>` on the Groceries add/edit form (rule 12: a field with real answers offers them, never a fixed list that refuses the true one — and unlike a `<datalist>`, whose suggestion popover is unreliable on mobile Safari, the options are an actual dropdown a phone can open). An optional field passes `emptyLabel` ("Not recorded"), a real first option submitted as empty, so an answer once given can be cleared again, not only replaced (rule 12) — used by a transaction's payee/kind and a member's gender |
| `Stepper` | A small whole number moved with − and + ("how many children?"), for a count too small to deserve a keyboard (onboarding sheet, Family basics). The value is a real hidden form field, so the form posts it without script; each button says what it does ("Fewer children", rule 10's "never a control that says nothing"), and the number is announced as it changes |
| `ChoiceChips` | A short, known set of answers shown all at once as chips — a work schedule, the days a helper comes — when seeing every option helps more than a menu (rule 20: a choice is picked, not typed). Real radio buttons or checkboxes under the chip styling, so the group posts without script and arrows move within a radio group; chips wrap, never clip (rule 15). A longer or open-ended set stays a `Select`/`ComboboxField` |
| Onboarding frame and screens (app, `apps/web/app/onboarding/`, story 02-009) | Setup is one task at a time, so it has no navigation: a back chevron, "Step 2 of 4" with its bar, and — only on the screens with no "I'll do this later" of their own — a close that does exactly that, so the two are never both on screen (rule 14). The people steps share one tab row (Adults · Children · Pets · Help) whose tabs *save and then move*, so switching never loses what was typed. Review rows are rule 4's shape: tile, the responsibility, one line of why these owners, then the owner picker and a Keep chip (unticked means "not for us", and it is not suggested again). The summary's rows are ready or carry their one way in; a ready row has no chevron, which is itself the message. Home's resume card is the whole row as one link with "Continue" drawn inside it, not a second button beside it |
...
| `ChoiceList` | One answer from a long, known list shown in full: a language, a region, a currency (story 22-003, rule 20). It is a search box above real radio buttons, so the form posts without script and arrow keys move through it. Each option carries its own `lang` and `dir`, so a language's native name is shaped by the right font and direction whatever the page's language. The selected option shows its check, and a search with no matches says so rather than showing an empty box. A short list stays `ChoiceChips`, and a list only an Admin may change is shown read-only instead |
| `DocumentLocale` (shell) | Sets `<html lang>` and `dir` from the viewer's language. The root layout stays static, because reading a cookie there would make every page dynamic. For the same reason the shell uses logical properties (`ms-`/`me-`, `start-`/`end-`, `text-start`) and mirrors directional icons with `rtl:rotate-180`. The brand's own English (the wordmark and tagline) keeps `dir="ltr"` inside a right-to-left page, and `ScriptAccent` takes `dir="auto"` so an English line keeps its punctuation in place (rule 2) |
| `CurrencyField` (`apps/web/app/_components`) | A money record's currency: the household's currencies to pick from, starting at the household default, plus "Another currency…" for one not listed (rule 20). The default applies only to a new record. A record's own currency is never changed by a later household change, and nothing is converted (rule 22) |
| `PersonalizeCard` (`apps/web/app/_components`) | Home's one invitation into the optional localization setup, shown to someone who has not finished or dismissed it. It has one action, which resumes the setup where it was left, and one dismiss, which is itself a form post. It never appears while family setup is still showing on Home, so the two never compete (rule 14) |
| `ReminderRow` (`apps/web/app/_components/reminder-row.tsx`, story 23-005) | One reminder in the notification center, built on `ExpandableRow` (rule 21): the category's `IconTile`, the title, a priority word (High in amber, never red — red stays for the critical, rule 3), an unread dot and the time. Opened, it shows the live record behind it as a small definition list — never a copy stored on the reminder — then one line of why it came when it did, then the one committing action (Mark as paid / Mark done, in Primary Blue, only when this person may actually do it), a link to the record, "Remind me later" (`ChoiceChips` presets, then a picked day and time — rule 20) and a quiet Dismiss. The actions post and then redirect with a closed code, so the confirmation survives the row itself disappearing |
| `QuietHoursCard`, `ReminderPreferencesCard` (`apps/web/app/_components/reminder-settings-form.tsx`, story 23-007) | Notification settings as choices, never typed times: quiet hours are two quarter-hour `Select`s worded in the viewer's own time format, with the household's time zone named underneath; each kind of reminder is a tile row with an On checkbox and the policy's own presets. Each card is keyed on its saved values so it redraws with what was actually stored |
| `ReminderDigest` (`apps/web/app/_components/reminder-row.tsx`, story 23-011) | "HomeBrain summary · Today": a short card above the notification list naming today's reminders in the order they come, each with its category tile (small) and time. Built from the very rows below it, so it can never say something the list does not (rule 9), shown only with more than one thing, and hidden for anyone who turned it off. A summary, never a second list to act from — the rows below keep the actions (rule 14) |
| `SmartRemindersCard` and "What comes to you" (`reminder-settings-form.tsx`, `settings/notifications/page.tsx`, stories 23-009, 23-011, 23-012) | Two plain checkboxes — the day's summary, and learning when the person usually acts — each saying what it does and what it never does ("a timing you chose above always wins"). Beside it, the responsibilities that route reminders to this person, first or as backup, as rows of name plus one line of why, with one link to change them on the Responsibilities screen rather than a second editor here (rule 14) |
| Grouped school day in `ReminderRow` (story 23-008) | "Aarav — 3 things for tomorrow" opens to each item with its due date, marked "(done)" once it is, and one committing action that says its count ("Mark all 3 done") — never a button that would only do part of what the row names |
| `SidebarLink` `badge` | A count of what is new beside a secondary-nav entry (the drawer's and sidebar's Notifications), as a small primary pill with an `aria-label` ("3 new") so the number is never colour or position alone. The label gives way to it rather than being clipped (rule 11) |
