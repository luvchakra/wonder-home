...
| `AddTaskMenu` | Home's own quick-add launcher (the mockup's "+ Add task" pill): a dropdown naming each domain's own existing add flow (homework, grocery item, a meal plan, a bill) — never a new add path invented here, and never a second "Ask AI" door (rule 13): every option is a manual add flow that screen already owns. `AddTaskOption.icon` takes an already-rendered node (an `IconTile`), since the menu is a Client Component and can't accept a raw icon-component reference from server code |
| `FamilyIllustration` | A second small original illustration (rule 8) — three family figures and a heart, for Home's greeting header, alongside a speech-bubble `ScriptAccent` ("Happier Homes Happier Humans!") |
