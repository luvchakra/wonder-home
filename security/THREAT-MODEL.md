...
| T19 | Prompt injection through household content | Deterministic authorization outside the model. `authorizeToolCall` re-checks scope, entitlement, permission and autonomy on every call; the tool list offered to a model is a convenience, never the control; refusals are recorded | **Built** — `tools.test.ts`, `scripts/test-agent-runs-rls.mjs` |
